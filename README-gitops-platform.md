...
| 10 | **Secrets via `secrets.auto.tfvars` (git-ignored) + `.env` for Ansible** | Hardcoded values, Vault | Sensitive material never enters git. Terraform reads tfvars; Ansible reads env vars in Option A, skipping interactive prompts in CI. |
| 11 | **Vector → Quickwit for logging, Prometheus/Grafana/Kiali for metrics/traces** | Loki/ELK, Jaeger | Lightweight, lab-friendly stack: single-agent (Vector) ships logs; Quickwit handles scale without a full Elasticsearch cluster; Kiali complements Istio for service-mesh observability. |
| 12 | **Proxmox VE as the hypervisor** | VirtualBox, VMware ESXi, bare-metal KVM scripts | Proven open-source KVM platform with a clean API that Terraform drives directly (clone-from-template, cloud-init, IP assignment). No licensing cost, and it scales from a lab toward a larger homelab without switching tools. |
| 13 | **GitHub Actions + GHCR as the CI/CD pipeline** | Jenkins, GitLab CI, Tekton, self-hosted Harbor | Lives in the same repository as the code (`.github/workflows/pipeline.yml`), zero self-hosted CI server to operate, and matrix/path filters are built in — matching the repo-centric GitOps model. Images are pushed to GHCR (free for the project, fine-grained PAT auth, no hardcoded credentials) because the lab has no spare registry host; a local Harbor remains an option if it must work offline. |
| 14 | **Security gate before build: Gitleaks + SonarQube, Trivy before push** | Post-deploy scanning, lint-only | A failing gate skips the builds entirely, so a leaked secret or a CRITICAL/HIGH image vulnerability never reaches the registry or the cluster. SARIF output lands in the Security tab for review. |
| 15 | **Kyverno as the cluster policy engine** | OPA Gatekeeper, manual review | Kyverno policies are plain YAML written in native Kubernetes syntax (no Rego needed), enforced cluster-wide for pod security and image requirements. |
| 16 | **External Secrets Operator for in-cluster secrets** | Hand-written Kubernetes Secrets, sealed-secrets | Secrets live in the backend (not as git-tracked Secret objects) and ESO syncs them into the cluster; the GitOps repo stays free of sensitive material and drift self-heals on reconcile. |
...
Example scaled cluster (`cluster_node_counts = { super = 3, master = 3, worker = 2 }`):
...
- Ansible group names must use underscores (`k3s_masters`), not hyphens — hyphens trigger warnings and unpredictable behavior in `hostvars`/`groups` (role names like `load-balance` are unaffected)
